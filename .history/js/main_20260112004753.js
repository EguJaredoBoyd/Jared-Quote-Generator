...
//Create the HTML icon m-dash variable
const mDash = document.getElementById("m-dash");

// Load the saved quote if it exists
const savedQuote = JSON.parse(localStorage.getItem("lastQuote"));

if (savedQuote) {
  const apiData = savedQuote;
  quoteText.textContent = `<span><i class="ri-double-quotes-l"></i></span>apiData.quote<span><i class="ri-double-quotes-r"></i></span>`;
...
async function fetchQuote() {
  ...
  try {
    ...
    localStorage.setItem("lastQuote", JSON.stringify(apiData));
  } catch (error) {
    console.error(`Error fetching quote: ${error}`);
    quoteText.textContent = "Sorry!, failed to quote.";
    quoteAuthorId.textContent = "";
    mDash.textContent = "";
  } finally {
